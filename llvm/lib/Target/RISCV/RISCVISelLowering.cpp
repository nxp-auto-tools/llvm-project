...
SDValue RISCVTargetLowering::LowerOperation(SDValue Op,
  ...
  case ISD::STORE:
    if (Subtarget.hasStdExtZilsd() &&
        ...
        if (!Subtarget.hasFastUnalignedAccess() && cast<StoreSDNode>(Op)->getAlign() < 8)
          return SDValue();
...
void RISCVTargetLowering::ReplaceNodeResults(SDNode *N,
  ...
  case ISD::LOAD: {
    if ((N->getValueType(0) == MVT::i64 || N->getValueType(0) == MVT::v2i32) && Subtarget.hasStdExtZilsd()) {
        ...
        // is not aligned to a value bigger than 8.
        if (!Subtarget.hasFastUnalignedAccess() && cast<LoadSDNode>(N)->getAlign() < 8)
          return;
...
static MachineBasicBlock *emit_ZILSD_SD(MachineInstr &MI, MachineBasicBlock *BB) {
  ...
  if (MI.getOperand(0).getReg() == RISCV::X0_X0 &&
      MI.getOperand(1).getReg() == RISCV::X0_X0)
