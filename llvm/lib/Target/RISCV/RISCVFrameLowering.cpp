...
void RISCVFrameLowering::emitPrologue(MachineFunction &MF,
  ...
  for (const auto &Entry : CSI) {
    ...
    if (STI.hasFeature(RISCV::FeatureStdExtZilsd) &&
      ...
      Registers.push_back(STI.getRegisterInfo()->getSubReg(Reg, RISCV::sub_32));
      Registers.push_back(
          STI.getRegisterInfo()->getSubReg(Reg, RISCV::sub_32_hi));
    } else
...
bool RISCVFrameLowering::assignCalleeSavedSpillSlots(
  ...
  if (STI.hasFeature(RISCV::FeatureStdExtZilsd) &&
    ...
    DenseMap<Register, bool> PairCalleeSavedRegs = {
        std::make_pair(RISCV::X8_PD, false),
        std::make_pair(RISCV::X18_PD, false),
        std::make_pair(RISCV::X20_PD, false),
        std::make_pair(RISCV::X24_PD, false),
        std::make_pair(RISCV::X26_PD, false)};
    ...
    for (auto &CS : CSI) {
      ...
      Register SuperRegLo =
          TRI->getMatchingSuperReg(Reg, RISCV::sub_32, &RISCV::GPRPRegClass);
      Register SuperRegHi =
          TRI->getMatchingSuperReg(Reg, RISCV::sub_32_hi, &RISCV::GPRPRegClass);
      ...
      Register PairedSubReg = SuperRegLo
                                  ? TRI->getSubReg(SuperRegLo, RISCV::sub_32_hi)
                                  : TRI->getSubReg(SuperRegHi, RISCV::sub_32);
